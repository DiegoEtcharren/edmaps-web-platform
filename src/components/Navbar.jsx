...
export default function Navbar({ currentPage, onNavigate }) {
  ...
  const handleLink = (e, page, hash) => {
    e.preventDefault();
    if (onNavigate) onNavigate(page);
    setIsOpen(false);
    if (hash) {
      // Gently delay hash navigation to ensure DOM is mounted
      setTimeout(() => {
    ...
    <nav className="fixed top-0 left-0 w-full h-[60px] glassmorphism z-50 flex items-center px-6 md:px-12 justify-between">
      ...
      {isOpen && (
        <div className="absolute top-[60px] left-0 w-full bg-white border-b border-slate-steel/10 flex flex-col p-6 space-y-4 md:hidden shadow-2xl animate-fade-in-down z-40">
          <a href="#hero" onClick={(e) => handleLink(e, 'home', '#hero')} className="text-secondary hover:text-industrial-navy font-bold tracking-label-caps py-2">INICIO</a>
          <a href="#telemetry" onClick={(e) => handleLink(e, 'home', '#telemetry')} className="text-secondary hover:text-industrial-navy font-bold tracking-label-caps py-2">TELEMETRÍA</a>
          <a href="#operative" onClick={(e) => handleLink(e, 'home', '#operative')} className="text-secondary hover:text-industrial-navy font-bold tracking-label-caps py-2">PISO DE OPERACIÓN</a>
          <a href="#genealogy" onClick={(e) => handleLink(e, 'home', '#genealogy')} className="text-secondary hover:text-industrial-navy font-bold tracking-label-caps py-2">TRAZABILIDAD</a>
          <a href="#contacto" onClick={(e) => handleLink(e, 'contact', null)} className="text-secondary hover:text-industrial-navy font-bold tracking-label-caps py-2">CONTACTO</a>
